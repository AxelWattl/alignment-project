...
class PIController:
    """Simple PI controller for laser alignment.

    Kp keeps the original proportional behavior. Ki slowly corrects
    small leftover steady-state error when the beam is close to target.
    """
    def __init__(self, kp=1.0, ki=0.0, integral_limit=200.0):
        self.kp = float(kp)
        self.ki = float(ki)
    ...
    def compute(self, error_x, error_y):
        ...
        # Anti-windup: keep accumulated error from growing forever.
        ...
        control_x = self.kp * error_x + self.ki * self.integral_x
        control_y = self.kp * error_y + self.ki * self.integral_y
        return control_x, control_y

def adjust_hardware_alignment(oUSB, strDeviceKey, delta_x, delta_y, actuator_num, pi_controller=None):
    ...
    # Use PI only when close to the iris center.
    # Far away, use the original P-only behavior so the integral term
    # does not wind up while the beam is still making large moves.
    if pi_controller is not None and error_distance < 30:
        control_x, control_y = pi_controller.compute(delta_x, delta_y)
    else:
        if pi_controller is not None:
            pi_controller.reset()
        control_x, control_y = delta_x, delta_y

    steps_x = int(control_x * STEPS_PER_PIXEL_X * GAIN)
    steps_y = int(control_y * STEPS_PER_PIXEL_Y * GAIN)
...
class HardwareThread(QThread):
    ...
    def __init__(self):
        ...
        # PI controllers for automatic alignment.
        # Start with small Ki values. Increase slowly only if the beam
        # stops with a small remaining offset from the iris center.
        self.pi_m1 = PIController(kp=1.0, ki=0.02, integral_limit=200.0)
        self.pi_m2 = PIController(kp=1.0, ki=0.01, integral_limit=200.0)
                    ...
                    if self.is_aligning:
                        ...
                        if l1_x is None:
                            ...
                            if time.time() > self.alignment_cooldown:
                                ...
                                cooldown = adjust_hardware_alignment(self.oUSB, self.strDeviceKey, err1_x, err1_y, 1, self.pi_m1)
                            ...
                            if time.time() > self.alignment_cooldown:
                                ...
                                cooldown = adjust_hardware_alignment(self.oUSB, self.strDeviceKey, err2_x, err2_y, 2, self.pi_m2)
                                self.alignment_cooldown = time.time() + cooldown
...
class AlignerApp(QMainWindow):
    ...
    def start_alignment(self):
        ...
        self.hw_thread.pi_m1.reset()
        self.hw_thread.pi_m2.reset()
